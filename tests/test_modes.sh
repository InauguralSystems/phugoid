...
# and exactly 180 checks ran (the pinned population — fewer means a
...
grep -q '^CHECKS_RUN 180$' "$OUT" || { echo "FAIL: check population not 180"; tail -3 "$OUT"; exit 1; }
grep -q '^FAILURES 0$' "$OUT" || { echo "FAIL: failures reported"; grep '^FAIL' "$OUT"; exit 1; }
echo "PASS: 180/180 oracle checks green"
