...
# and exactly 173 checks ran (the pinned population — fewer means a
...
grep -q '^CHECKS_RUN 173$' "$OUT" || { echo "FAIL: check population not 173"; tail -3 "$OUT"; exit 1; }
grep -q '^FAILURES 0$' "$OUT" || { echo "FAIL: failures reported"; grep '^FAIL' "$OUT"; exit 1; }
echo "PASS: 173/173 oracle checks green"
