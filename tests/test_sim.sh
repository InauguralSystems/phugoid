...
# bridge is valid in the sim's regimes, and exactly 61 checks ran.
set -euo pipefail
...
grep -q '^CHECKS_RUN 61$' "$OUT" || { echo "FAIL: check population not 61"; tail -3 "$OUT"; exit 1; }
grep -q '^FAILURES 0$' "$OUT" || { echo "FAIL: failures reported"; grep '^FAIL' "$OUT"; exit 1; }
echo "PASS: 61/61 rung-1 checks green"
