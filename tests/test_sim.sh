...
# bridge is valid in the sim's regimes, and exactly 79 checks ran.
set -euo pipefail
...
grep -q '^CHECKS_RUN 79$' "$OUT" || { echo "FAIL: check population not 79"; tail -3 "$OUT"; exit 1; }
grep -q '^FAILURES 0$' "$OUT" || { echo "FAIL: failures reported"; grep '^FAIL' "$OUT"; exit 1; }
echo "PASS: 79/79 rung-1 checks green"
