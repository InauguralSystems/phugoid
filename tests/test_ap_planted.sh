...
# 116-check population, and the manifest rules hold (identity incl.
...
run_plant() {
    ...
    grep -q '^CHECKS_RUN 116$' "$OUT" || { echo "FAIL: plant $1 run population != 116"; exit 1; }
}
expect_total_fails() {
...
run_plant s3; expect_total_fails 30
...
run_plant s4; expect_total_fails 71
...
run_plant s7; expect_total_fails 53
...
run_plant s9; expect_total_fails 57
...
run_plant s12; expect_total_fails 61
