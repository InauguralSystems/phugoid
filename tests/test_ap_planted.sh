...
# 92-check population, and the manifest rules hold (identity incl.
...
run_plant() {
    ...
    grep -q '^CHECKS_RUN 92$' "$OUT" || { echo "FAIL: plant $1 run population != 92"; exit 1; }
}
expect_total_fails() {
...
run_plant s4; expect_total_fails 57
...
run_plant s7; expect_total_fails 34
...
run_plant s9; expect_total_fails 45
...
run_plant s12; expect_total_fails 38
