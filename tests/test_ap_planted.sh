...
# 96-check population, and the manifest rules hold (identity incl.
...
run_plant() {
    ...
    grep -q '^CHECKS_RUN 96$' "$OUT" || { echo "FAIL: plant $1 run population != 96"; exit 1; }
}
expect_total_fails() {
...
run_plant s1; expect_total_fails 43
...
run_plant s2; expect_total_fails 43
...
run_plant s3; expect_total_fails 21
...
run_plant s4; expect_total_fails 60
...
run_plant s7; expect_total_fails 36
...
run_plant s9; expect_total_fails 46
...
run_plant s12; expect_total_fails 42
