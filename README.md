...
- **`measure.eigs`** — the estimators that will grade rung-1+ trajectories:
  ...
  spans), aperiodic t½ by exponential fit. 109 pinned checks against
...
- **`ORACLE.md`** — the quality bar itself: published numbers, stated
  tolerances (with written justifications for every widening), a sixteen-plant
...
bash tests/test_measure.sh     # 109 estimator checks vs synthetic truth
bash tests/test_planted.sh     # 16 plants must each flip exactly their checks
```
